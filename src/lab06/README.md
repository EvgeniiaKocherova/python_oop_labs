...
### TypedCollection[T] — типизированная коллекция
- `TypedCollection[Book]` — коллекция только для книг
- `TypedCollection[Student]` — коллекция только для студентов

**Основные методы:**
- `add(item: T)` — добавление элемента указанного типа
...
Создание `TypedCollection[Book]`, добавление объектов разных типов из иерархии (Book, AudioBook, EBook). 
...
- Поиск несуществующей книги — возвращается `None`

### Скриншот работы

![Поиск элемента](../../images/lab06/img02.png)

### Сценарий 3. Метод filter() — фильтрация коллекции

Применение разных условий фильтрации:
- Книги до 1900 года — возвращается список из одной книги
- Книги длиннее 300 страниц — возвращается список из трёх книг

### Скриншот работы

![Фильтрация коллекции](../../images/lab06/img03.png)

### Сценарий 4. Метод map() — преобразование с изменением типа
...
### Сценарий 5. Комбинация filter() и map()

Применение цепочки преобразований:
- Сначала `filter()` отбирает книги после 1900 года
- Затем выводятся названия отфильтрованных книг
...
### Класс Library (lab04/collection.py)
Методы коллекции аннотированы:
- `add(book: Book) -> None`
